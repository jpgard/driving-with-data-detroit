...
def bayesian_prop_test(successes, n, rope=0.01, plot=False):
    ...
    :return: posterior probability that the means of the two groups are not the same;
    that is: P(mu_1 != mu_2 | data)
    """
    ## for testing:
    with pm.Model() as model:
        ...
        trace = pm.sample(2000, tune=100,
                          # note: these converge relatively quickly; not much sampling
                          # needed
                          cores=1)  # note: must set cores=1 because using Python2.7;
        # see https://github.com/pymc-devs/pymc3/issues/3255
    if plot:
    ...
    trace_theta_diff_in_rope = np.apply_along_axis(lambda x: abs(x[0] - x[1]) <= rope, 1,
                                                   theta_posterior_samps)
    p_rope = sum(trace_theta_diff_in_rope) / float(len(theta_posterior_samps))
    return diff_in_posterior_means, p_rope


def freq_seq_search(seqs, systems, search_min_support=180, search_max_seqs=500,
                    search_min_seqs=5, search_min_length=2,
    ...
    :param seqs:
    :param systems:
    :param search_min_support: minimum support to consider; will be iteratively lowered
    if insufficient sequences are found (best to start with very large value;
    overestimating induces negligible additional computation).
    ...
    freq_seqs = list()
    all_systems_in_freq_seqs = False  # indicator for whether at least one frequent
    # subsequence has been found with every system in systems
    while len(freq_seqs) < search_max_seqs:
        freq_seqs = seqmining.freq_seq_enum(seqs,
                                            search_min_support)  # set of (sequence,
        # frequency) tuples
        freq_seqs = [x for x in freq_seqs if
                     len(x[0]) >= search_min_length and not set(x[0]).isdisjoint(systems)]
    ...
    if add_all_system_ngrams:
        ...
        for ngram_length in range(ngram_range[0], ngram_range[1] + 1):
            print(
                "[INFO] getting all system ngrams of length {} with min_support {}".format(
                    ngram_length, ngram_min_support))
            ngram_obs = [ngrams(s, ngram_length) for s in seqs.tolist()]
            # "unroll" the ngram iterators, keeping only sequences which contain one of
            # systems
            system_ngram_counts = Counter(
                [b for vehicle_bigrams in ngram_obs for b in vehicle_bigrams if
                 not set(b).isdisjoint(systems)])
            for system in systems:
                system_ngrams = sorted(
                    [x for x in system_ngram_counts.items() if
                     system in x[0] and x[1] >= ngram_min_support],
                    key=lambda x: x[1], reverse=True)
                freq_seqs += system_ngrams
    end_time = time.time()
    print(
        "[INFO] found frequent sequences in {} sec".format(round(end_time - start_time)))
    return freq_seqs
...
def i_ratio(left_seqs, right_seqs, systems, identifier, stat_test_method):
    ...
    :param identifier: vehicle name or other identifier for this specific testing
    iteration.
    :param stat_test_method: technique to use for statistical inference; either
    "frequentist" or "bayesian".
    :return:
    """
    output_data = []
    left_freq_seqs = freq_seq_search(left_seqs, systems)
    print("[INFO] Identified {} frequent sequences for {}; conducting testing".format(
        len(left_freq_seqs), identifier))
    for left_seq in left_freq_seqs:
        ...
        if stat_test_method == "frequentist":
            # for frequentist stat_test_method, test_statistic_value is a z-score; p is
            # its p-value
            # t test for difference between two population means for
            # left_seq_norm_support and right_seq_norm_support
            test_statistic_value, p = proportions_ztest(counts, nobs, value=0.05)
        elif stat_test_method == "bayesian":
            # for bayesian stat_test_method, test_statistic_value is the magnitude of
            # the difference in posterior means theta_0 - theta_1;
            ...
            raise NotImplementedError(
                "stat_test_method must be either 'frequentist' or 'bayesian'.")
        seq_data = (identifier, seq, left_seq_support, round(left_seq_norm_support, 4),
                    right_seq_support,
                    round(right_seq_norm_support, 4), round(i_ratio, 2),
                    round(test_statistic_value, 1), round(p, 4))
...
def create_i_ratio_df(left_seqs, right_seqs, systems, identifier, stat_test_method):
    ...
    i_ratio_results = i_ratio(left_seqs, right_seqs, systems, identifier,
                              stat_test_method=stat_test_method)
    if i_ratio_results:  # i_ratio() returns no results if frequent sequences cannot be
        # found
        results_df = pd.DataFrame(i_ratio_results)
        results_df.columns = ['Identifier', 'Sequence', 'Left Support',
                              'Left Norm Support', 'Right Support',
                              'Right Norm Support', 'i-Ratio', 'z', 'P(z)']
        results_df.sort_values(by=['Identifier', 'P(z)', 'Left Support'],
...
def compute_cluster_membership(A):
    """
    Using BGMM, compute two clusters (in-group = 1, out-group = 1) based on each
    individual column of A.
    :param A: PARAFAC factor loading matrix A.
    :return: np.array with binary values, with 1 indicating in-group membership and 0
    otherwise.
    """
    n, R = A.shape
...
def run_prism(A_matrix_fp, vehicle_ingroup_matrix_fp, B_matrix_fp,
              system_ingroup_matrix_fp,
    ...
    :param vehicle_lkp_fp: path to vehicle lookup df; used to ensure ordering matches A
    matrix precisely.
    :param rgrid: grid of values to evaluate for r; by default all factors in range(R)
    will be evaluated.
    :return:
    """
    ...
    print("[INFO] writing vehicle cluster membership matrix to {}".format(
        vehicle_ingroup_matrix_fp))
    np.savetxt(vehicle_ingroup_matrix_fp, vehicle_ingroup_matrix)
    print("[INFO] writing system cluster membership matrix to {}".format(
        system_ingroup_matrix_fp))
    np.savetxt(system_ingroup_matrix_fp, system_ingroup_matrix)
    print("[INFO] writing time cluster membership matrix to {}".format(
    ...
    for r in rgrid:
        ...
        left_maint_seq_df = generate_vehicle_maintenance_seq_df(write_to_file=False,
                                                                filter_col=time_colname,
                                                                filter_values=in_group_times)
        left_seqs = left_maint_seq_df[left_maint_seq_df["unit"].isin(in_group_uids)][
            "maint_seq"]
        right_seqs = maint_seq_df[~maint_seq_df["unit"].isin(in_group_uids)]["maint_seq"]
        identifier = "PARAFAC_{}".format(r)
        i_ratio_df = create_i_ratio_df(left_seqs, right_seqs, in_group_systems,
                                       identifier, stat_test_method="bayesian")
        if i_ratio_df is not None:
            outpath = './freq-pattern-data/i_ratios_{}_PARAFAC_r{}.csv'.format(
                time_colname, r)
...
def concatenate_maintenance_sequences(seqs, start_token="<START>", end_token="<END>"):
    """
    Concatenate all subsequences in seqs into a single sequence, with start_token and
    end_token indicating the beginning and ending of subsequences.
