...
def i_ratio(maint_dict, vehicle, min_support = 180, min_seqs = 5, min_length = 3):
    ...
    for left_seq in left_freq_seqs:
        ...
        seq_data = (vehicle, seq, left_seq_support, round(left_seq_norm_support, 4), right_seq_support, round(right_seq_norm_support, 4), round(i_ratio, 2), round(z_stat, 1), round(p_z, 4))
...
def main(v_list = ['HUSTLER_X-ONE', 'SMEAL_SST_PUMPER', 'DODGE_CHARGER', 'FORD_CROWN_VIC'], outpath = './freq-pattern-data/i_ratios.csv'):
    ...
    i_ratio_df = create_i_ratio_df(maint_dict=maint_dict, v_list=v_list)
