...
if (analysis_type == 'month_year_log'){
    ...
    cluster_membership_matrix = read.csv("./tensor-data/month_year/bgmm_ingroup.txt", sep = " ", header = F)
}
...
for(i in seq(ncol(A))) {
    message(paste0("Generating plot for component ", i))
     ...
     a_plot = ggplot(A, aes(x = factor(row.names(A)), y = A[,i], fill = temp$Dept.Desc)) + 
         ...
         theme(plot.title = element_text(hjust = 0.5, size = rel(2)),panel.border = element_rect(colour = "black", fill=NA, size=1),panel.background = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank(), axis.title.x = element_blank(), legend.text = element_text(size = rel(0.4)), legend.key.size = unit(0.15, units = "cm"),legend.position=c(0.98,0.15), legend.justification=c(1,0)) + 
         ylim(0,1) + 
         ggtitle("Vehicle") + 
     ...
     outfile = paste0('img/3_way_plots/', subfolder, '/3_way_plot_factor_update_', i, '.', OUT_DEVICE)
