...
def cross_prod(df, cols):
    # return a single dataframe with cross product of values in cols; should only be passed 2 columns
    assert len(cols) == 2
    to_cross = []
    for col in cols:
    ...
    df_out = pd.merge(to_cross[0], to_cross[1], on = 'key', how = 'outer').drop(['key'], axis = 1)
    df_out.columns = cols
    return df_out
...
def factorize_column(df, col, outfile):
    # factorizes categorical column, encoding using numeric ids instead; writes a lookup table to outfile
...
def make_pre_tensor(df, lkp_suffix, modes = ['Unit#', 'System Description', 'year_WO_Opened']):
    # create 3-way tensor with dimensions of maintenance_df_modes
    mode_1 = modes[0]
    mode_2 = modes[1]
    mode_3 = modes[2]
    # make dummies for each category of mode_2; then create df that has an entry for every (mode_1,mode_3) combo
    ...
    # convert each variable to take numeric id values, starting at 1, instead of original values
    # mode_1
    pre_tensor = factorize_column(pre_tensor, col = mode_1, outfile = './tensor-data/{1}/{0}_{1}_lkp.csv'.format(re.sub('[^0-9a-zA-Z]+', '', mode_1), lkp_suffix))
    # mode_2
    pre_tensor = factorize_column(pre_tensor, col = 'variable', outfile = './tensor-data/{1}/{0}_{1}_lkp.csv'.format(re.sub('[^0-9a-zA-Z]+', '', mode_2), lkp_suffix))
    # mode_3
